...
- [ ] Привести все UI-суммы и проценты к единому формату `100 000,00` /
...
Эти направления не назначены конкретным версиям. Их порядок требует отдельного
решения после owner acceptance текущего среза.

## Долги

- [ ] Входящие долги: `owed_to_me`.
- [ ] Исходящие долги: `i_owe`.
- [ ] Контрагент, первоначальная и текущая сумма, срок возврата.
- [ ] Частичное погашение и связь с финансовой операцией.
- [ ] Учёт в календаре и прогнозе.

## Кредиты и рассрочки

- [ ] Создание кредита или рассрочки.
- [ ] Первоначальная и текущая сумма.
- [ ] Регулярный платёж, периодичность и следующая дата платежа.
- [ ] Кредитор и упрощённый график платежей.
- [ ] Подтверждение платежа и уменьшение остатка обязательства.
- [ ] Учёт в прогнозе.

## CSV/Excel-импорт

- [ ] Загрузка CSV с определением кодировки и разделителя.
- [ ] Загрузка XLSX и выбор листа.
- [ ] Общий pipeline сопоставления колонок, нормализации и preview.
- [ ] Выбор целевого счёта и поиск вероятных дублей.
- [ ] Явное подтверждение импорта и отчёт о результате.

## Ежедневная работа и аналитика

- [ ] Шаблоны и дублирование операций.
- [ ] Улучшенный поиск и сохранённые фильтры.
- [ ] Массовое изменение категорий.
- [ ] Дальнейшая мобильная адаптация и более удобный календарь.
- [ ] Настраиваемый dashboard и план-факт анализ.
- [ ] Динамика остатков, движение по фондам и улучшенные отчёты.
- [ ] Состояние долгов и кредитов после реализации этих доменов.
- [ ] Предложение категории на основе истории без внешнего AI-сервиса.
