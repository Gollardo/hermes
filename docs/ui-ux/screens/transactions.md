...
Предварительная спецификация журнала для `0.1.0-alpha.3` и последующего
развития. Владелец подтвердил важность серийного ручного ввода и будущего
импорта. Документ не утверждает tags, audit trail, saved filters или bulk edit
до соответствующих продуктовых и доменных решений.
...
- заметки, если описание останется опциональным, и tags только после утверждения
  этих понятий;
- selection totals и bulk actions как развитие доказанного сценария.
...
- Фонд добавляется после реализации фондов.
- Активные условия видимы как chips и отражаются в заголовке результата.
- Сброс возвращает явный default, например текущий месяц, а не неизвестное
  состояние.
- Количество и сумма результатов относятся ко всей выборке, а не только к
  странице.
...
Desktop использует таблицу. Mobile показывает сгруппированный список: смысл и
сумма в первой строке, дата/счёт/категория — ниже. Полная деталь открывается
отдельным слоем; критичные поля не прячутся в горизонтальном scroll.
