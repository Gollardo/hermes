...
- на узком экране навигация превращается в компактный многорядный верхний блок:
  ...
  списки и формы перестраиваются в один смысловой поток;
...
- Экранные суммы группируют тысячи пробелом и показывают минимум
  два десятичных знака (`100 000.00`). Значимые третья и четвёртая цифры
  сохраняются, поэтому форматирование не округляет точное доменное значение.
