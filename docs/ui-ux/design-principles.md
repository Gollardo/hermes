...
Принципы являются предварительной основой для прототипов. Они не утверждают
конкретный layout, библиотеку компонентов или визуальную тему. При конфликте с
подтверждёнными доменными инвариантами приоритет остаётся у доменной
документации.
...
- После форматирования числовые поля группируют тысячи пробелами, сохраняя в API
  точную decimal-строку без разделителей.
...
в фондах + свободно». Валюта и знак видимы; округлённый график раскрывает точное
значение; отрицательная сумма имеет текстовую семантику, а не только красный
цвет.
