...
explicit events. The allocation generated by the composed physical
transfer-and-allocation command is the exception: it records the transfer as
its cause and is reversed only when that transfer is deleted. The ordinary
operation editor rejects editing this composite because it cannot represent the
