...
- Owner decision 2026-08-18 задаёт единый будущий UI-контракт чисел: все суммы
  ...
  равнозначный десятичный разделитель. Это пока документированный контракт:
  полное соответствие текущего frontend не реализовано и не проверено.
...
- Документационный аудит 2026-08-18 повторно подтвердил 70 passed и 53
  PostgreSQL-сценария skipped без opt-in для `make test-backend`, 100/100
  frontend-тестов в 20 файлах, `make lint`, `make typecheck`, `make docs-check`
  и production Angular build. PostgreSQL, Docker build и браузерные сценарии в
  этом повторном прогоне не запускались; их последний полный снимок приведён
  ниже.
